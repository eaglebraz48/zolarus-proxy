...
export default function ShopCTA({ size = 'md' }: { size?: 'sm' | 'md' }) {
  ...
  function goCompare(e: React.MouseEvent) {
    e.preventDefault();
    ...
    window.location.href = `/compare?${qp.toString()}`;
  }

  // If user is not on /shop yet, send them there first to fill inputs
  const onShopPage = mounted ? window.location.pathname.startsWith('/shop') : false;

  if (isActive) {
    return onShopPage ? (
      <a href="#" onClick={goCompare} style={{ ...common, background: '#059669', color: '#fff' }}>
        {T[lang].open}
      </a>
    ) : (
      <a
        href={`/shop?lang=${encodeURIComponent(lang)}`}
