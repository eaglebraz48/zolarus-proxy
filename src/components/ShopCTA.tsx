...
const T: Record<Lang, { open: string; subscribe: string; loading: string; signin: string }> = {
...
export default function ShopCTA({ size = 'md' }: { size?: 'sm' | 'md' }) {
  ...
  if (isActive) {
    return (
      <a href={`/shop?open=compare&lang=${lang}`} style={{ ...common, background: '#059669', color: '#fff' }}>
        {T[lang].open}
      </a>
    );
  }
  ...
  return (
    <a href="#subscribe" onClick={startCheckout} style={{ ...common, background: '#0f172a', color: '#fff' }}>
      {T[lang].subscribe}
    </a>
  );
}
